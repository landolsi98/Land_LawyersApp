...
@AnonymousAllowed
@Route(value = "dash test" , layout = MainView.class)
public class DashTest extends VerticalLayout {
    ...
    private void saveAbogado(TeamForm.SaveEvent evt) {
        abogadoService.add(evt.getAbogado());
        closeEditor();
        updateList();
    ...
    private void deleteAbogado(TeamForm.DeleteEvent event) {
        ...
        dialog.addConfirmListener(e -> {
            ...
            abogadoService.delete(event.getAbogado());
            closeEditor();
            updateList();
        });
    ...
    public DashTest(@Autowired AbogadoService abogadoService) {
        this.abogadoService = abogadoService;
        this.form = new TeamForm();

        // Setup  grid with columns for title, date, and body
        grid.setColumns("idAbogado", "firstName", "lastName", "email", "specialty", "phoneNumber", "barNumber", "position","description");
        grid.setItems(abogadoService.findAll());
